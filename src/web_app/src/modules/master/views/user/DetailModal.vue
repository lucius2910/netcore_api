<template>
  <vc-modal ref="modal" :title="tl('User', 'DetailTitle')" width="70%">
    <template #content>
      <el-form ref="userForm" :model="user" :rules="rules" label-width="150px" label-position="right">
        <vc-row :gutter="20">
          <vc-col :lg="12" :md="12" :sm="24" :xs="24">
            ...
            <vc-input-group required prop="email" :label="tl('User', 'Email')">
            ...
            <vc-input-group required :label="tl('User', 'Gender')">
              <vc-select v-model="user.gender" fieldValue="value" fieldText="name" :items="genders"></vc-select>
            </vc-input-group>
          ...
          <vc-col :lg="12" :md="12" :sm="24" :xs="24">
            <vc-input-group :label="tl('User', 'Role')">
              <vc-select v-model="user.roles" multiple :items="roles" fieldValue="code" fieldText="name">
              </vc-select>
            </vc-input-group>
          </vc-col>
        </vc-row>
    ...
    <template #acttion>
      <vc-button type="primary" class="ml-2" code="F00004" @click="onSave(userForm)" :loading="isLoading"
        :icon="FolderChecked">
        {{ tl("Common", "BtnSave") }}
      </vc-button>
